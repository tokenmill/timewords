...
run-tests-jdk-9:
	docker build -f Dockerfile.jdk9.test -t timewords:jdk9 . && docker run timewords:jdk9
...
run-tests-from-8-to-11-till fail:
	make run-tests-jdk-8 && make run-tests-jdk-9 && make run-tests-jdk-10 && make run-tests-jdk-11
